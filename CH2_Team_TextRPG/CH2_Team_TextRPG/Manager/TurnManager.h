...
public:
	 ...
	 */
	void AddCharacterToTimeline(Character* character);

	/**
	 * @brief 대기열 정렬 후 다음 턴 캐릭터 반환
	 * @return 다음 턴 캐릭터에 대한 포인터.
	 */	
	Character* GetNextTurnCharacter();

	/**
	 * @brief 캐릭터의 행동 수치를 초기화
	 * @param character 행동 수치를 초기화할 캐릭터에 대한 포인터.
	 */
	void ResetCharacterTurn(Character* character);
	 ...
	 */
	void AddActionValue(Character* character, float value);
private:
	 ...
	 */
	float GetCharacterSpeed(Character* character) const;
